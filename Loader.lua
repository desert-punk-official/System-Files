...
-- Version: 21.4 (Added Auto-Environment & Persistence)
...
local ENV_FILE_NAME = "punk-x-env.txt" -- Stores Stable/Beta preference
...
StableBtn.MouseButton1Click:Connect(function()
    ...
    SaveEnvironmentPreference("STABLE") -- Save choice
    CloseLoader()
...
if saved then
    ...
    if saved == SECRET_DEV_KEY then
        ...
        if env_preference == "BETA" then
            ...
            Blur:Destroy()
            return -- STOP HERE
        elseif env_preference == "STABLE" then
            Notify("Punk X", "Auto-Loading Main...", 3)
        ...
        if success and valid then 
            ...
            -- Public users usually default to Stable, or if they selected it before
            if env_preference == "STABLE" then
                Notify("Punk X", "Auto-Loading...", 3)
