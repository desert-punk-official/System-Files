...
-- Version: 21.6 (Updated for New Panda API v1)
...
local UI_CONFIG = {
    Title = "PUNK X",
    Version = "v21.7", -- 🔴 UPDATED VERSION (Added New UI button)
...
SelectionContainer.Size = UDim2.new(0.85, 0, 0.6, 0) -- 🔴 BIGGER: 0.6 instead of 0.4 to fit 3 buttons
...
OldUIBtn.Size = UDim2.new(1, 0, 0.28, 0) -- 🔴 Each button is ~28% height
...
NewUIBtn.Position = UDim2.new(0, 0, 0.36, 0) -- 🔴 36% from top (0 + 28% + 8% gap)
...
BetaBtn.Position = UDim2.new(0, 0, 0.72, 0) -- 🔴 72% from top (36 + 28 + 8)
...
AddButtonEffects(OldUIBtn) -- 🔴 Renamed from StableBtn
AddButtonEffects(NewUIBtn) -- 🔴 NEW BUTTON
...
local function ShowLauncherMenu(isDev)
    ...
    if isDev then
        ...
        -- Keep original sizes (3 buttons layout)
        ...
        -- 2 buttons layout (bigger buttons, centered)
        OldUIBtn.Size = UDim2.new(1, 0, 0.45, 0)
        OldUIBtn.Position = UDim2.new(0, 0, 0, 0)
        NewUIBtn.Size = UDim2.new(1, 0, 0.45, 0)
...
OldUIBtn.MouseButton1Click:Connect(function()
    ...
    SaveEnvironmentPreference("OLD") -- 🔴 Changed from "STABLE" to "OLD"
    CloseLoader()
...
NewUIBtn.MouseButton1Click:Connect(function()
    ...
    SaveEnvironmentPreference("NEW") -- 🔴 NEW BUTTON
    CloseLoader()
...
BetaBtn.MouseButton1Click:Connect(function()
    ...
    SaveEnvironmentPreference("BETA") -- Save choice
    CloseLoader()
...
RedeemBtn.MouseButton1Click:Connect(function()
    ...
    if key == SECRET_DEV_KEY then
        ...
        isChecking = false -- 🔴 ADDED: Reset flag
        return 
    end
    ...
    if success and valid then
        ...
        SetExpiryData(data) -- 🔴 This now handles the new format
    ...
    isChecking = false -- 🔴 ADDED: Reset flag
    RedeemBtn.Text = "Redeem" -- 🔴 ADDED: Reset button text
end)
...
if saved then
    ...
    if saved == SECRET_DEV_KEY then
        ...
        if env_preference == "BETA" then
            ...
            Blur:Destroy()
            return -- STOP HERE
        elseif env_preference == "OLD" or env_preference == "STABLE" then -- 🔴 Support both OLD and STABLE
            Notify("Punk X", "Auto-Loading Old UI...", 3)
            LaunchPunkX(SECRET_DEV_KEY, Main_URL)
            ScreenGui:Destroy()
            Blur:Destroy()
            return -- STOP HERE
        elseif env_preference == "NEW" then -- 🔴 NEW PREFERENCE
        ...
        if success and valid then 
            CURRENT_KEY = saved
            SetExpiryData(data) -- 🔴 Handles new format

            -- Public users can use OLD or NEW UI
            if env_preference == "OLD" or env_preference == "STABLE" then -- 🔴 Support both OLD and STABLE
                Notify("Punk X", "Auto-Loading Old UI...", 3)
